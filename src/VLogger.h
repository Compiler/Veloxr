...
// [LR] The beginning of allowing C++ like qInfo() << ...; to work with frontend logging stream.
// Jk. I hate switching between qInfo() & console.log. Might come back if front end and back end logging
// isn't synced.
class PrefixSuffixStreamBuffer : public std::streambuf {
    public:
        PrefixSuffixStreamBuffer(std::ostream& stream, const std::string& prefix, const std::string& suffix)
            : stream_(stream), prefix_(prefix), suffix_(suffix) {}

    protected:
        virtual int_type overflow(int_type c) override {
            if (c != traits_type::eof()) {
                std::string output = prefix_ + static_cast<char>(c) + suffix_;
                return stream_.rdbuf()->sputn(output.c_str(), output.length());
            }
    ...
    class PrefixSuffixStream : public std::ostream {
    public:
        PrefixSuffixStream(const std::string& prefix, const std::string& suffix)
            : std::ostream(&buffer_), buffer_(*this, prefix, suffix) {}

    private:
        PrefixSuffixStreamBuffer buffer_;
    };
